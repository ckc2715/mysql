...
function PacketWriter(encoding) {
  this._buffer = null;
  this._offset = 0;
  this._encoding = encoding || 'BIG5'
}
...
PacketWriter.prototype.writeNullTerminatedString = function (value, encoding) {
  ...
  value = value || '';
  value = value + '';
  var _tempBuffer = iconv.encode(value, this._encoding)
  var bytes = _tempBuffer.length + 1

  this._allocate(bytes);

  _tempBuffer.copy(this._buffer, this._offset);
